...
(defun add-help-field (args)
  (unless (assoc "help" (remove-if-not #'listp args))
    ;; Here we need to intern help symbol into the package expanding
    ;; the macro
    (push (list (alexandria:ensure-symbol "HELP") "Show help on this program." :flag t)
          args)))
...
(defun get-command-name (symbol)
  (let* ((package (symbol-package symbol))
    ...
    (cond ((cl-strings:starts-with package-name
                                   roswell-prefix)
           (elt (cl-strings:split package-name ".")
                2))
...
(defun make-binding (name &rest args)
  ...
  `(,name (net.didierverna.clon:getopt :long-name ,(string-downcase (symbol-name name)))))
...
"
  (let ((bindings (map-fields #'make-binding defmain-args))
    ...
    ;; 
    (when rest-arg
      (push `(,rest-arg (net.didierverna.clon:remainder))
            bindings))
...
(defmacro defmain (name (&rest args) &body body)
  (let* ((command-name (get-command-name name))
    ...
    `(progn
       (net.didierverna.clon:defsynopsis (,@synopsis-args)
         ,@docstring
         ,@synopsis-fields)
       ...
       (defun ,name (&rest argv)
         ...
         (if (net.didierverna.clon:getopt :long-name "help")
             ...
             (let (,@bindings)
               ...
               (declare (ignorable ,(alexandria:ensure-symbol 'help)))
               ,@body))))))
